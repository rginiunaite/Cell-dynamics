...
for Number =1
    ...
    t_final = 500; % time interval for Gillespie algorithm
    ...
    if Number == 1 && rescaled_time(length(rescaled_time)) ~= 0 % ignore the solutions where time jumps back to zero and starts moving again
        save('gillespie1D_large_initial_Pp05_Pd01_40G_long_L10000.mat','save_time','save_density','save_cA0','save_store_time');
        S = load('gillespie1D_large_initial_Pp05_Pd01_40G_long_L10000.mat');
    end
    
    if Number == 2 && rescaled_time(length(rescaled_time)) ~= 0
        save('gillespie1D_Pp05_Pd01_40G_long_L10000.mat','save_time','save_density','save_cA0','save_store_time');
        S = load('gillespie1D_Pp05_Pd01_40G_long.mat');
    end    
    
    if Number == 3 && rescaled_time(length(rescaled_time)) ~= 0
        save('gillespie1D_Pp1_Pd05_40G_long_L10000.mat','save_time','save_density','save_cA0','save_store_time');
        S = load('gillespie1D_Pp1_Pd05_40G_long.mat');
    end
