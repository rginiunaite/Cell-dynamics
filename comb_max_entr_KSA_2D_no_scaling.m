% HEXAGONAL DOMAIN: combination logistic and Runge-Kutta with KSA + maximum entropy in 2D
...
S = load('gillespie2D_large_initial_Pp06_Pd04_hexa_40G.mat'); % load the data for a particular choice of Pp and Pd, note that L=100 is fixed
...
% hold on
% plot(S.save_time,y_log_new,'LineWidth', 3)
...
function deriv = dynamics(t,y,L,Pm,Pp,Pd,gammaAA,gammaA0)
    ...
    deriv(3) = 1/3 * Pm * (y(2) + 2 * y(3) +3 * y(4)- 6*y(3)) - ...
        2 * Pd * y(3) + 1/3 * Pp * (gammaA0(2).*(gammaA0(1)).*(gammaAA(1)+2*gammaAA(2)+3*gammaAA(3))); % 
    
   % deriv(4) = 1/3 * Pm * (y(3) + 2 * y(4) +3 * y(5)- 6*y(4)) - ...
    %    2 * Pd * y(4) + 1/3 * Pp * (gammaA0(3).*(gammaA0(1)).*(gammaAA(1)+2*gammaAA(3)+3*gammaAA(4))); % 

    deriv(3:L) = 1/3 * Pm * (y(2:L-1) + 2 * y(3:L) +3 * y(4:L+1)- 6*y(3:L)) - ...
            2 * Pd * y(3:L) + 1/3 * Pp * (y(1)*ones(L-2,1) - y(3:L)).* (y(1)*ones(L-2,1) - y(2)*ones(L-2,1)).*(y(2:L-1)+2*y(3:L)+3*y(4:L+1))/(y(1)^2*(1-y(1))); % 
    
    deriv(L+1) = 1/3 * Pm * (y(L) + 2 * y(L+1) +3 * y(1)^2- 6*y(L+1)) - ...
            2 * Pd * y(L+1) + 1/3 * Pp * (y(1) - y(L+1))* (y(1) - y(2))*(y(L)+2*y(L+1)+3*y(1)^2)/(y(1)^2*(1-y(1))); % Periodic boundary conditions
