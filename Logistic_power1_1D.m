% combination of Gillespie (40 realisations), logistic and Runge-Kutta with power-1 (normal-moment clusure) approximation in 1D
...
h_legend = legend('Gillespie 1D','Logistic growth','Power 1');
set(h_legend,'FontSize',14)
xlabel('rescaled time','FontSize',14)
ylabel('rescaled density','FontSize',14)
...
function deriv = dynamics(t,y,L,Pm,Pp,Pd)
    ...
    deriv(2) = (Pm*(y(3) - y(2)) - 2* Pd * y(2) + ...
        Pp * (y(1) - y(2)) + Pp * (y(3)*(1-y(1)) + (y(1)-y(2))*y(1)-2*y(1)^2*(1-y(1)) + y(1)*(y(1) - y(2)))); % dynamics of two-point distribution function (distance 1)
    
 % dynamics of all two-point functions from distance 2 to L-1
    for i = 3:L
        deriv(i) = (Pm*(y(i-1) + y(i+1) - 2* y(i)) - 2 * Pd * y(i) +...
            Pp * ((y(i+1)+y(i-1))*(1-y(1)) + 2 * ((y(1)-y(i))*y(1) - 2*y(1)^2*(1-y(1))+y(1)*(y(1)-y(2)))));
    end

    deriv(L+1) = (Pm*(y(L) + y(2) - 2* y(L+1)) - 2 * Pd * y(L+1) +...
            Pp * ((y(1)+y(L))*(1-y(1)) + 2 * ((y(1)-y(L+1))*y(1) - 2*y(1)^2*(1-y(1))+y(1)*(y(1)-y(2))))); % for the last one, since we have
