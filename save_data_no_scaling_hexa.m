% combination of Gillespie on a hexagonal lattice in 2D
% save date for different parameter values, no scaling
...
for Number =1:2
    ...
    if Number == 1 && store_time(length(store_time)) ~= 0 % ignore the solutions where time jumps back to zero and starts moving again
        save('gillespie2D_large_initial_Pp06_Pd04_hexa_40G.mat','save_time','save_density','save_cA0','save_store_time');
        S = load('gillespie2D_large_initial_Pp06_Pd04_hexa_40G.mat');
    end
    
    if Number == 2 && store_time(length(store_time)) ~= 0
        save('gillespie2D_large_initial_Pp04_Pd06_hexa_40G.mat','save_time','save_density','save_cA0','save_store_time');
        S = load('gillespie2D_large_initial_Pp04_Pd06_hexa_40G.mat');
    end    
