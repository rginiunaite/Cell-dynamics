...
% maximum entropy approximation + KSA in 1D, using fsolve for step 3 in maximal
% entropy scheme
...
S = load('gillespie1D_large_initial_Pp06_Pd04_40G_long.mat'); % load the data for a particular choice of Pp and Pd, note that L=100 is fixed
...
new_time = S.save_time;%
new_density = S.save_density;%
% figure 
% 
% plot(new_time,new_density,'LineWidth', 3);set(gca,'FontSize',14); % number of cells in the system
...
rescaled_time = (Pp-Pd)*S.save_time;
...
% hold on
% plot(new_time,y_log_new,'LineWidth', 3)
% xlabel('time','FontSize',14)
% ylabel('density','FontSize',14)
...
N =round(S.save_time(length(1:round(length(S.save_time)/TimeReductionFactor)))/dt); % number of simulations, chosen to be the same as for Gillespie
...
rescaled_time = store_time_Runge; % rescale time to allow parameter comparison
...
function deriv = dynamics(t,y,Ln,Pm,Pp,Pd,gammaAA,gammaA0)
 ...
 %           Pp * ((y(1)-y(Ln+1))*(y(1)-y(2))*(y(Ln)+y(Ln)))/(y(1)^2*(1-y(1)))); % for the last one, since we have
     ...
     deriv(Ln+1) = (Pm*(y(Ln) + y(1)*y(1) - 2* y(Ln+1)) - 2 * Pd * y(Ln+1) +...
             Pp * ((y(1)-y(Ln+1))*(y(1)-y(2))*(y(Ln)+y(1)*y(1)))/(y(1)^2*(1-y(1))));  
