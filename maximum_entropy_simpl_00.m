...
S = load('gillespie1D_large_initial_Pp05_Pd01_40G.mat'); % load the data for a particular choice of Pp and Pd, note that L=100 is fixed
...
% initial conditions for gamma, first L/2 correspond to gamma(A,A), the other L/2, gamma(A,0)
    ...
    for i = 1:L-1
         ...
         gamma00(i) = S.save_cA0; % chosen without any good reason
    end
...
function y = step3AA(gammaAA,gammaA0,p2,Ln,cA)
    ...
    y(1:Ln) = p2 - gammaAA(1:Ln).*(gammaA0(1:Ln).*gammaAA(1:Ln) + gammaA0(1:Ln).*gammaA0(1:Ln));
