...
N = 200; % round(S.save_store_time(length(1:round(length(S.save_store_time)/TimeReductionFactor)))/dt); % number of simulations, chosen to be the same as for Gillespie
...
for i = 2 : N
     ...
     [xA0,fval] = fsolve(@(gamma_unk)step3A0(gamma_unk,gammaAA(:,count),y(2:L+1,final),L,y(1,final)),gammaA0(:,count),options);
...
function y = step3AA(gammaAA,gammaA0,p2,Ln,cA)
  ...
  y(1:Ln) = p2 - gammaAA(1:Ln).*(gammaA0(1:Ln).*gammaAA(1:Ln) + gammaA0(1:Ln).*gammaA0(1:Ln));%+gammaAA(1:Ln) +gammaA0(1:Ln));
    
end


function y = step3A0(gammaA0,gammaAA,p2,Ln,cA)
    

    y = zeros(1,Ln);
        
        
        y(1:Ln) = cA*ones(Ln,1) - p2 - gammaA0(1:Ln).*(gammaAA(1:Ln).*gammaA0(1:Ln)+1.5*gammaA0(1:Ln).*gammaA0(1:Ln));%+gammaA0(1:Ln)+gammaA0(1:Ln));     
