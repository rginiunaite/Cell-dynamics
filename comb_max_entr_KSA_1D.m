...
% maximum entropy approximation (for short distances) + KSA (for long distances) in 1D, using fsolve for step 3 in maximal
% entropy scheme
...
S = load('gillespie1D_Pp1_Pd05_40G_long.mat'); % load the data for a particular choice of Pp and Pd, note that L=100 is fixed
...
N = 2*round(S.save_time(length(1:round(length(S.save_time)/TimeReductionFactor)))/dt); % number of simulations, chosen to be the same as for Gillespie
...
xlabel('rescaled time','FontSize',14)
ylabel('rescaled density','FontSize',14)
title(['P_p = ' num2str(Pp) ', P_d = ' num2str(Pd) ' '],'FontSize',14,'FontWeight','Normal')
set(gca,'linewidth',3)
...
function deriv = dynamics(t,y,Ln,Pm,Pp,Pd,gammaAA,gammaA0)
 ...
 % indix i+1 correspond to distance i, therefore, gamma indices are smaller
   ...
   deriv(3) = Pm * (y(2) + y(4) - 2*y(3)) - 2 * Pd * y(3)+ ...
        Pp*(gammaA0(2).*(gammaA0(1)).*(gammaAA(1)+gammaAA(3)));
    
   % deriv(4) = Pm * (y(3) + y(5) - 2*y(4)) - 2 * Pd * y(4)+ ...
     %   Pp*(gammaA0(3).*(gammaA0(1)).*(gammaAA(2)+gammaAA(4)));
 
         % dynamics of all two-point functions from distance 2 to L-1
    
  %  deriv(3:Ln) =  (Pm*(y(2:Ln-1) + y(4:Ln+1) - 2* y(3:Ln)) - 2 * Pd * y(3:Ln) +...
   %         Pp * ((y(1)*ones(Ln-2,1)-y(3:Ln)).*(y(1)*ones(Ln-2,1)-y(2)*ones(Ln-2,1)).*(y(2:Ln-1)+y(4:Ln+1)))/(y(1)^2*(1-y(1))));
    ...
    deriv(Ln+1) = (Pm*(y(Ln) + y(Ln) - 2* y(Ln+1)) - 2 * Pd * y(Ln+1) +...
            Pp * ((y(1)-y(Ln+1))*(y(1)-y(2))*(y(Ln)+y(Ln)))/(y(1)^2*(1-y(1)))); % for the last one, since we have
        % periodic boundary condition L+1 neighbour is 1 neighbour, i.e.
        % y(2), wrong, L
...
function y = step3AA(gammaAA,gammaA0,p2,Ln,cA)
    ...
    %y(1:Ln) = p2 - gammaAA(1:Ln).*(gammaAA(1:Ln).*gammaAA(1:Ln) + gammaA0(1:Ln).*gammaA0(1:Ln));
...
function y = step3A0(gammaA0,gammaAA,gamma00,p2,Ln,cA)
   ...
   % y(1:Ln) = cA*ones(Ln,1) - p2 - gammaA0(1:Ln).*(gammaAA(1:Ln).*gammaA0(1:Ln)+gamma00(1:Ln).*gammaA0(1:Ln));   
...
function y = step300(gamma00,gammaA0,gammaAA,p2,Ln,cA)
    ...
    %y(1:Ln) = ones(Ln,1) - 2*cA*ones(Ln,1) + p2 - gamma00(1:Ln).*(gammaA0(1:Ln).*gammaA0(1:Ln)+gamma00(1:Ln).*gamma00(1:Ln));   
